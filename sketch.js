...
function draw(){
    ...
    stroke("black");
    textSize(30);
    text("Press or click the 'START' button to play!",windowWidth/2-280,windowHeight/2-250);
    textSize(20);
    text("Press the UP ARROW KEY or keep tapping the screen and strive to keep Doraemon in air!",windowWidth/2-400,windowHeight/2-150);
    ...
    if(gameState===PLAY){
      ...
      cakes();
      if(touches.length>0&&player.y>150||keyDown(UP_ARROW)&&player.y>150){
      player.velocityY=-9;
      touches=[];
      }
